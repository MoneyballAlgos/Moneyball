...
def socket_setup(log_identifier='Cron'):
    ...
    socket_thread = threading.Thread(name=f"Streaming-{now.strftime('%d-%b-%Y %H:%M:%S')}", target=connect_to_socket, args=(correlation_id, mode, sws, subscribe_list), daemon=True)
    socket_thread.start()
